...
export default class FormSearch<T, U> extends React.Component<BaseFormProps<T, U> & { ghost?: boolean }> {
    ...
    render = () => {
        ...
        columns = columns.filter(col => col.hideInSearch !== true && col.valueType !== 'option')
        ...
        return <Card style={{marginBottom: 16}}>
            ...
            >
                {columns.map((col, index) => {
                    let label = col.title as React.ReactNode;
                    let Field = getField(col.valueType)

                    return <Form.Item label={label} name={col.dataIndex} key={index}>
                        <Field mode='edit'></Field>
                    </Form.Item>
                })}
